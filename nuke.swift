...
func runCommand(_ command: String, arguments: [String] = [], log: Bool = true) {
...
func executeSchema(_ schema: Schema, arguments: [String]) {
    ...
    for process in schema.processes {
        terminateProcess(matching: process)
    }
    ...
    for command in schema.commands {
        ...
        if components.count >= 1 {
            ...
            runCommand(cmd, arguments: args)
        }
    }

    print("🧨 \(schema.name) nuked")
}
...
func run(arguments: [String], availableSchemas: [Schema]) {
    guard arguments.count > 1 else {
        print("""
        Usage: nuke <command> [--help]

        Available commands:
        \(availableSchemas.map { "  \($0.name)" }.joined(separator: "\n"))
        """)
        exit(1)
    }
    ...
    guard let schema = availableSchemas.first(where: { $0.name == input }) else {
...
// Entry point
run(arguments: CommandLine.arguments, availableSchemas: parseAllSchemas(schemasPath: "schemas"))
